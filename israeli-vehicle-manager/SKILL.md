...
7. **Wartime licence extensions are not a reservist deferral.** During the Iron Swords war, and again during the later operations against Iran, the Ministry of Transport extended vehicle and driving licences automatically by order, but only for licences that expired inside specific published windows. The windows and extension lengths differ per period, some extensions applied to all vehicle owners, and some extra time applied only to security-forces personnel, evacuees and their spouses. None of the published windows is a standing deferral keyed to reservist service, so do not promise one. Before telling a user a licence or test deadline was extended, check the expiry date against the current list of windows on Kol Zchut or the Ministry of Transport site; for anything outside a window, the user should contact the Ministry of Transport service line, where case-by-case extensions are sometimes granted.

# Israeli Statistics Skill — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build a skill for accessing Israeli Central Bureau of Statistics (CBS) data — price indices (CPI, housing), economic indicators, demographic data, and statistical publications.

**Architecture:** MCP Enhancement + Domain-Specific Intelligence skill. Enhances the existing `israel-statistics-mcp` server (8 tools) with CBS methodology knowledge, index interpretation guidance, and economic context.

**Tech Stack:** SKILL.md, references for CBS data tables, index methodologies, and publication schedules.
...
- **Repository:** Community MCP server for Israeli CBS data (by Reuven Aor)
- **Tools (8):** Dedicated tools for querying CBS statistical tables and data series
- **Enhancement opportunity:** Add index interpretation, methodology context, and economic analysis guidance

### CBS (Halishka Hamerkazit Listatistika)
- **URL:** `https://www.cbs.gov.il`
- **English URL:** `https://www.cbs.gov.il/en`
- **Role:** Israel's official national statistics office
- **Data portal:** Statistical tables, publications, press releases
- **API:** CBS provides structured data access for statistical tables
- **Publication schedule:** Regular monthly, quarterly, and annual releases

### Consumer Price Index (CPI / Madad Hamchirim Latarchan)
- **Published:** Monthly, around 15th of following month
- **Base period:** Updated periodically (current base varies by series)
- **Components:** Food, housing, transportation, health, education, clothing, etc.
- **Uses:** Rent adjustments (linked rental contracts), salary negotiations, economic policy
- **Known as "hamadad"** — one of the most closely watched economic indicators in Israel
- **Housing component:** Separately tracked as "madad hadiyur"

### Housing Price Index (Madad Mchirei Dirot)
- **Published:** Quarterly by CBS, based on Tax Authority transaction data
- **Coverage:** All residential property transactions nationwide
- **Breakdowns:** By city, district, apartment size, new vs. existing
- **Importance:** Key indicator for real estate market, mortgage planning, and policy

### Producer Price Index (Madad Mchirei Yatzran)
- **Published:** Monthly by CBS
- **Coverage:** Prices received by domestic producers for their output
- **Sectors:** Manufacturing, agriculture, mining
- **Uses:** Tracks production cost trends, feeds into CPI forecasting

### Other Key CBS Data Series
| Series | Hebrew | Frequency | Key Use |
|--------|--------|-----------|---------|
| GDP (Tatzar Megulmi Golmi) | tatsar megulmi golmi | Quarterly | Economic growth |
| Unemployment rate | shiur haavtala | Monthly | Labor market |
| Population estimates | ochilosia | Annual/quarterly | Demographics |
| Immigration (aliyah) | aliya | Monthly | Demographic trends |
| Building starts | hatchalot bniya | Monthly | Construction sector |
| Exports/Imports | yetzu/yivu | Monthly | Trade balance |
| Industrial production | tatzur taasiyati | Monthly | Manufacturing sector |
| Wage statistics | schar avoda | Quarterly | Income trends |
| Poverty data | oni | Annual | Social indicators |

### Use Cases
1. **CPI lookup** — Current and historical consumer price index values
2. **Rent adjustment** — Calculate rent increase based on CPI change
3. **Housing prices** — Track residential property price trends by area
4. **Economic indicators** — GDP growth, unemployment, trade balance
5. **Demographic data** — Population by city, age, sector
6. **Custom data query** — Access specific CBS statistical tables
...
description: >-
  ...
  about Israeli statistics, "hamadad", CPI, consumer price index, housing prices,
  "madad mchirei dirot", GDP, unemployment, population data, CBS data, "halishka
  hamerkazit listatistika", producer prices, building starts, or any Israeli
  economic/demographic statistics. Enhances israel-statistics-mcp server with
  index interpretation and economic context. Do NOT use for non-Israeli
...
metadata:
  ...
  tags: [statistics, cpi, housing-prices, economics, demographics, cbs, israel]
...
| Food | mazon | ~16% |
| Health | briut | ~6% |
| Education & culture | chinuch vetarbut | ~8% |
| Clothing & footwear | halbasha vehanala | ~3% |
| Furniture & household | rihut umeshek bayit | ~5% |
| Other | acher | ~20% |

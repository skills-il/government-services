...
- **Eligibility:** age 20-67; 12 of the 18 months before first registering worked as an employee. Dismissal, resignation or employer-imposed chalat all qualify; the reason only sets when payment starts.
- **Resignation:** paid only from 90 days after leaving unless the cause is justified (list: `references/benefit-programs.md`). A contract ending is not a resignation: paid from the first reporting day. Separately, every claimant, fired or resigned, is unpaid for the first 5 days of unemployment in each run of 4 consecutive reporting months; those days do not reduce the benefit quota.

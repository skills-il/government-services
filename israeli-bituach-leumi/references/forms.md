# Bituach Leumi Forms Catalog (טפסים)
...
| Form | Hebrew name | Filed by | Notes |
|---|---|---|---|
| 270 | תביעה למתן שיקום מקצועי | Disabled person, work-injured, widow(er) on survivors pension, terror victim | Single unified rehab claim covering all sub-programs |
...
When a court orders alimony and the obligor doesn't pay, BTL collects on behalf of the recipient.

| Form | Hebrew name | Filed by | Notes |
|---|---|---|---|
| 5400 | בקשה לתשלום על פי חוק המזונות | Recipient with court-ordered alimony | Main claim |
| 5409 | הצהרה על פי חוק המזונות | Existing recipient | Annual declaration of family/financial status changes |
| 5422 | בקשה להפסקת או חידוש מזונות | Recipient | Stop or resume payments |

## Employer bankruptcy (zchuyot ovdim be'pshitat regel)

BTL pays unpaid wages, severance, and pension contributions when a court issues a bankruptcy/liquidation/insolvency order against the employer.

| Form | Hebrew name | Filed by | Notes |
|---|---|---|---|
| 5305 | תביעת עובד לתשלום חוב שכר עבודה ופיצויי פיטורים | Employee | Main worker claim. Requires court order against employer |
| 5310 | תביעה לתשלום חוב לקופת גמל | Pension fund | Fund claims employer pension debts |
| 5312 | שאלון למי שאינו תושב ישראל | Non-resident worker | Filed alongside 5305 if claimant is not an Israeli resident |

## Burial (dmei kvura)

| Form | Hebrew name | Filed by | Notes |
|---|---|---|---|
| 460 | תביעה אישית לדמי קבורה | Family of deceased | Used when no authorized burial body exists in the locality, or when buried abroad. Standard chevra-kadisha cases are reimbursed institutionally without a claim form |
...
| 2640 | הצהרת תובע לאחר שחרור מבית חולים סיעודי / מוסד | Claimant | Returning home after >90 days in institutional care; restores eligibility |
| 2666 | הסכמה להעברת תביעה לגמלת סיעוד באמצעות בית חולים | Hospitalized claimant | Initiate siyud claim during hospitalization |
...
| 595 | בקשה לוועדת חריגים לנפגעי פעולות איבה | Hostility-victim claimant (incl. hostage families) | Exceptions-committee review; also used by orphans of fatalities for marriage / age-30 grant |
| 596 | בקשה לוועדת חריגים או לסיוע לפנים משורת הדין | Bereaved families | Ex-gratia assistance |

> **Hostage families (Iron Swords):** the dedicated program for hostage and missing-person families routes through forms 595 and 596 plus the existing hostility-victim set (580/571/etc.). There is no standalone hostage form number; coordinate via the hostage-families program portal at btl.gov.il/HaravotBarzel1/HtofimHazro.
...
## Living abroad and bilateral treaty forms

Israeli pension recipients living abroad must submit annual life-certificate forms. Treaty-country recipients use country-specific forms.

| Form | Hebrew name | Filed by | Notes |
|---|---|---|---|
| 10420 | אישור חיים | Pension recipient living abroad | Annual life certificate (HE/EN bilingual). Applies to old-age, disability, survivors |
| A1003 | אישור חיים למוסד הסוציאלי בגרמניה | Recipient receiving German pension | Germany treaty form |
...
| 101 | ביטול סירוב לפניה למעסיקים ומשלמי פנסיה מוקדמת | Insured person | Cancel a previously filed Form 100 |
| 100 | סירוב לפניה למעסיקים ומשלמי פנסיה מוקדמת | Insured person | Opt-out from employer outreach (NOT a Tax Authority form 100) |
...
6. **Phone + WhatsApp:** *6050 (press 9 for English)

## Coverage gaps (no verifiable forms)

- **Hostage-specific application form:** program exists but routes through 595/596 + existing hostility-victim forms.

...
Per-orphan bases in special cases (parent in Israel but not entitled: 1,142 for a single child, 862 each when more than one; no surviving parent, or the parent lives abroad permanently: 1,142 per child; orphan of both parents: 2,284 per child). There is no cap on the number of eligible children. From 80, the 103 NIS age-80 addition (1,941 minus 1,838) is paid on top of the with-children rows too. A widow (not a widower) who had a child with her who has since left the definition is paid as if aged 50 (1,838), at any age.
...
Then add the **seniority increment (tosefet vetek)**: 2% of the base per full insurance year the deceased accrued (from the first year), up to a maximum of 50%. A full insurance year is 12 insurance months. Count the insurance years from the FIRST insurance year: since January 2019 there is no threshold, so never subtract the qualifying period before applying the 2%. Count only years in which the deceased was an insured worker (employee or self-employed) with contributions paid, not years as a non-working spouse; no increment is paid while contributions are in arrears.

A widow(er) **under 40 with no children** usually gets a one-time **survivor grant** instead: 36 allowances at the 1,838 rate (66,168 NIS, plus seniority), not 36 x 1,381. This is not absolute. The under-40 childless survivor still gets a **monthly** allowance if she/he is pregnant with, or later bears, the deceased's child (a shared child means she/he is treated as a widow(er) with a child, entitled from the date of death), if a widow had a child with her who has since aged out (paid as if 50), and, in the work-related-death (dependents') track only, if she could not support herself from work shortly before the death or within a year of it (paid as if 50). Inability to work does not create a monthly allowance in the ordinary track. So do not flatly rule out a monthly amount here: present the grant as the usual outcome, note these exception paths, and point to Bituach Leumi for the determination.

Run `scripts/estimate_survivor_allowance.py --age <age> --orphans <n> --seniority-years <y>` to produce the estimate (flags: `--widower`, `--child-aged-out`, `--posthumous-child`, `--orphans-alone`; work track: `--unable-to-support`, `--pregnant`; see `--help`). On a host that cannot run scripts (ZIP-upload platforms such as ChatGPT, Claude.ai, Claude Desktop or Manus), do not skip the estimate and do not report a tool error: compute it by hand from the table above, base plus 2% per insurance year capped at 50%. Present it clearly as an estimate only.

### Step 4: Work-related death branch (dependents' allowance)

If the death resulted from a work accident or occupational disease, the family claims the **dependents' allowance (kitzvat tluyim)** instead of the survivor's allowance. It is higher, has no income test for a widow (a childless widower qualifies only if he cannot support himself or his income is within the Table 9 ceiling), and you cannot receive both for the same death. It is a percentage of a full (100%) work-disability pension, which is 75% of the deceased's wage in the three months before the injury (capped so the full pension does not exceed 39,428 NIS):
...
Age exceptions on the widow(er) rows: a childless widow(er) under 50 is still paid the monthly 60% rate, at any age, when she could not support herself from work shortly before the death or within one year of it (a one-time grant already paid is then set off against the allowance); a widow (not a widower) is paid the same 60% when she had an entitled child with her at the death who has since aged out; and a widow who was pregnant at the death is treated as unable to support herself and paid 60% until the birth, then 80% (the widow-plus-one-child rate) afterwards.
...
- **Survivor grant (menak she'erim)**: one-time = 36 allowances at the 1,838 rate plus seniority, for a widow(er) under 40 with no children (and certain income cases); in the work track, 60% x 36 of the full work-disability pension.
...
- **Budgetary pension (pensiya taktzivit)**: if the deceased was a state, IDF, police or local-authority pensioner on a budgetary (non-funded) pension, the survivors' pension comes from the paying body under its own rules, not from a pension fund, and a one-time grant to the spouse may also apply. Ask which of the two tracks the deceased was on before sending anyone to a fund. Since 01.02.2026, under the Survivors' Pensions of Permanent-Service and State-Service Members Law (Legislative Amendments), 2026, a widow(er) on the budgetary survivors' pension of a career soldier or a state employee no longer loses it on remarriage. This includes a widow(er) who remarried earlier: payments run only from 01.02.2026 (none for earlier periods), and where a grant was paid under the old rules, from the end of the 18 or 36 months it covered, with a top-up for the months in between. A survivor newly entitled whose claim arose at least two years before 01.02.2026 must claim within two years of that date.
...
After the claim is approved, read `references/after-the-claim.md` for the rules on staying abroad (3 months, annual life certificate) and on income tax (the Bituach Leumi allowance is exempt; a fund or budgetary survivors' pension is exempt up to a ceiling, or in full where it derives from exempt payments).
...
- `references/after-the-claim.md`: staying abroad and the income-tax treatment of each survivors' payment.
...
- Do not confuse the **survivor's allowance (קצבת שאירים)** with the **dependents' allowance (קצבת תלויים)**: the work-related-death track only, higher, and you cannot receive both for the same death.
...
- Never use one flat allowance figure: the amount depends on the age band, the number of children and the seniority increment.
...
- A widow(er) on an old-age pension gets it IN FULL PLUS half the survivor's allowance; 919 is only half the childless-50+ base (see Step 3).
- Remarriage ends the allowance but not automatically everything: check the continuation and reinstatement rules in Step 5 before telling a widow(er) it is lost.

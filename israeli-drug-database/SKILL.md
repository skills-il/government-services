...
compatibility: >-
  Network access helpful for drug registry lookups. Enhanced by israel-drugs-mcp
  server.
metadata:
  author: skills-il
  version: 1.0.0
  ...
  display_name:
    ...
    en: Israeli Drug Database
  display_description:
    he: חיפוש במאגר התרופות של משרד הבריאות וסל הבריאות
    en: >-
      Query the Israeli pharmaceutical database for drug information, health
      basket coverage, generic alternatives, and pricing. Use when user asks
      about Israeli medications, "trufot", drug names, "sal briut" drug
      coverage, generic drugs, drug prices in Israel, prescription requirements,
      or medication safety info. Enhances israel-drugs-mcp server with health

# Israeli Drug Database Skill — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build a skill for querying the Israeli pharmaceutical database — drug lookup, health basket coverage status, generic alternatives, pricing, and drug safety information from the Ministry of Health drug registry.

**Architecture:** MCP Enhancement + Domain-Specific Intelligence skill. Enhances the existing `israel-drugs-mcp` server with health basket context, generic alternative mapping, and patient-facing drug information.

**Tech Stack:** SKILL.md, references for MoH drug registry, health basket formulary, and drug pricing data.
...
- **Repository:** Community MCP server for Israeli drug data
- **Function:** Queries the Ministry of Health pharmaceutical database
- **Enhancement opportunity:** Add health basket status, generic alternatives, pricing context, and safety information

### Ministry of Health Drug Registry
- **URL:** `https://data.health.gov.il/` (MoH data portal)
- **Drug database:** Full registry of approved pharmaceuticals in Israel
- **Data includes:**
  - Drug name (Hebrew and English, trade and generic)
  - Active ingredient (chomer peeil)
  - Manufacturer / marketing authorization holder
  - Registration number (mispar rishum)
  - Dosage forms and strengths
  - ATC classification code
  - Prescription status (prescription / OTC / restricted)
  - Registration status (active / suspended / cancelled)
- **Access:** Public, searchable online and via data.gov.il API

### Health Basket (Sal Briut) Drug Coverage
- **Definition:** The national formulary of medications that all health funds must provide
- **Updated:** Annually by the Health Basket Committee
- **Tiers:**
  - **Full coverage:** Fund pays full cost, patient pays minimal copay
  - **Partial coverage:** Fund pays percentage, patient pays remainder
  - **Not covered:** Patient pays full cost or obtains through supplementary insurance
- **Copay (hishtattfut atzmit):** Varies by drug category and health fund
  - Generic drugs: Lower copay
  - Brand-name drugs: Higher copay when generic available
  - Chronic condition drugs: Reduced copay for qualifying patients

### Drug Pricing in Israel
- **Regulated pricing:** Ministry of Health sets maximum retail prices
- **Reference pricing:** Based on international reference basket of countries
- **Generic pricing:** Typically 40-60% lower than brand-name equivalents
- **Pharmacy markup:** Regulated margin for pharmacies
- **Price list:** Published periodically by MoH (pricelist/mechron)

### Drug Safety Information
- **Side effects:** MoH maintains Hebrew patient information leaflets (aloney matan)
- **Drug interactions:** Cross-reference with active ingredients
- **Pregnancy categories:** Israeli classification aligned with international standards
- **Recall notices:** MoH publishes safety alerts and recalls
- **Yellow Card system:** Reporting adverse drug reactions to MoH Pharmacovigilance unit

### Use Cases
1. **Drug lookup** — Search by trade name, generic name, or active ingredient
2. **Health basket status** — Check if a drug is covered and at what tier
3. **Generic alternatives** — Find cheaper generic equivalents
4. **Drug pricing** — Look up regulated maximum price
5. **Safety information** — Side effects, interactions, pregnancy safety
6. **Prescription status** — Check if a drug requires prescription in Israel
